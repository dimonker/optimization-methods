const solveFibonacci = (a1, b1) => {
  ...
  function numbersFibo(n) {
    ...
    if (n <= 1) {
      ...
      let x1 = (1 + sqrt) / 2;
      let y1 = (1 - sqrt) / 2;
      let Fn = (1 / sqrt) * (Math.pow(x1, n) - Math.pow(y1, n));
      return Fn;
    }
  }
  ...
  function findLyamda(k) {
    let nforLymbda = n - k;
    let lyamda =
      a1 + (numbersFibo(nforLymbda - 1) / numbersFibo(n + 1)) * (b1 - a1);
    return lyamda;
  }
